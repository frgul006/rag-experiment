...
TRAINING_FILE_PATH = "./out/train.jsonl"
...
def init_pinecone(
    api_key: str, index_name: str, environment: str, dimension: int
) -> pinecone.Index:
    pinecone.init(api_key, environment=environment)
    if index_name not in pinecone.list_indexes():
        metadata_config = {"indexed": ["source"]}
        pinecone.create_index(
            index_name, dimension=dimension, metadata_config=metadata_config
        )
    return pinecone.Index(index_name)


def create_and_index_embeddings(
    data: list[dict[str, str]], model: str, index: pinecone.Index
) -> None:
    batch_size = 32
    for i in tqdm(range(0, len(data), batch_size)):
        ...
        embeds, metadata_batch = create_embeddings_and_metadata(
            text_batch, source_batch, model
        )
...
def create_embeddings_and_metadata(
    ...
    metadata_batch = [
        ...
        for text, source in zip(text_batch, source_batch)
    ]
...
def main() -> None:
    ...
    logger.debug(
        f"Loaded {len(training_data)} training examples from {TRAINING_FILE_PATH}"
    )

    logger.info("Initializing OpenAI model...")
    model = init_openai(OPENAI_API_KEY)
    sample_embedding = openai.Embedding.create(input="Doesn't matter", engine=model)[
        "data"
    ][0]["embedding"]
    ...
    pinecone_index = init_pinecone(
        PINECONE_API_KEY, PINECONE_INDEX_NAME, PINECONE_ENVIRONMENT, embedding_dimension
    )
